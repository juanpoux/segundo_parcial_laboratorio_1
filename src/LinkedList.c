...
static int addNode(LinkedList *this, int nodeIndex, void *pElement)
{
	...
	if(nodo != NULL && this != NULL && nodeIndex > -1 && nodeIndex <= ll_len(this))
	{
		...
		else
		{
			...
			nodo->pNextNode = auxAnterior->pNextNode;
			auxAnterior->pNextNode = nodo;
		}
		returnAux = 0;
		this->size++;
	}
...
int ll_remove(LinkedList *this, int index)
{
	...
	if(this != NULL && index > -1 && index < ll_len(this))
	{
		if(index == 0)
		{
			//revisar que pasa si borra la posicion 1
			nodo = this->pFirstNode;
